...
async function handlePayroll(request, env, me, requestId, url, path) {
  ...
  if (path === "/internal/api/v1/admin/payroll/preview") {
    ...
    try {
      ...
      try {
        await env.DATABASE.prepare("SELECT base_salary FROM Users LIMIT 1").first();
      } catch (_) {
        hasBaseSalary = false;
      }
      for (const u of users) {
        ...
        const hourlyBase = (baseSalary + regularAllowance) / 240;
        ...
        const otPay = hourlyBase * (1.34 * sums.weekday + 1.67 * sums.rest + 2 * sums.holiday);
