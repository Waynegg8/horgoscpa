...
var index_default = {
  async fetch(request, env) {
    ...
    if (path === "/internal/api/v1/cache-test") {
      const { generateCacheKey: generateCacheKey2, getCache: getCache2, saveCache: saveCache2 } = await Promise.resolve().then(() => (init_cache_helper(), cache_helper_exports));
      const corsHeaders = getCorsHeadersForRequest(request, env);
      try {
    ...
    if (path === "/internal/api/v1/cache-debug-clients") {
      ...
      const corsHeaders = getCorsHeadersForRequest(request, env);
      try {
        const cacheKey = generateCacheKey2("clients_list", { page: 1, perPage: 50, q: "", tag_id: "" });
