...
### [ ] 模組 3：客戶管理（客戶管理-完整規格.md）
**資料表：** 3 個 | **API：** 8 個 | **Cron Jobs：** 0 個

#### 3.1 資料表創建
- [ ] 3.1.1 創建 `Clients` 表（客戶資料）
- [ ] 3.1.2 創建 `CustomerTags` 表（客戶標籤）
- [ ] 3.1.3 創建 `ClientTagAssignments` 表（客戶與標籤關聯）

#### 3.2 客戶管理 API（小型事務所彈性設計：所有人可用）
- [ ] 3.2.1 實現 `GET /api/v1/clients` 路由（含 OpenAPI schema）
- [ ] 3.2.2 實現 `POST /api/v1/clients` 路由（含 OpenAPI schema）
- [ ] 3.2.3 實現 `GET /api/v1/clients/:id` 路由（含 OpenAPI schema）
- [ ] 3.2.4 實現 `PUT /api/v1/clients/:id` 路由（含 OpenAPI schema）
- [ ] 3.2.5 實現 `DELETE /api/v1/clients/:id` 路由（含 OpenAPI schema）

#### 3.3 標籤管理 API
- [ ] 3.3.1 實現 `GET /api/v1/clients/tags` 路由（含 OpenAPI schema）
- [ ] 3.3.2 實現 `POST /api/v1/clients/tags` 路由（含 OpenAPI schema）

#### 3.4 批量操作 API（僅管理員）
- [ ] 3.4.1 實現 `POST /api/v1/clients/batch-update` 路由（含 OpenAPI schema）
