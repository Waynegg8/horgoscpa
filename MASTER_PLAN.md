...
**當前進度：** 45/45 表，147/147 API（模組1-14 全部完成，🔄 審查修正中）
...
**9.6.3 文件大小和格式驗證**
- [x] 9.6.3.1 實現圖片大小驗證（最大 5MB）[規格:L641]
- [x] 9.6.3.2 實現資源文件大小驗證（最大 10MB）[規格:L642]
- [x] 9.6.3.3 實現文件格式驗證（image/*, PDF, Excel, Word, ZIP）
- [x] 9.6.3.4 實現圖片尺寸獲取（width, height）[規格:L494-L495]
...
**9.7.1 API 清單驗證**
- [x] 9.7.1.1 驗證 Blog 文章管理 API（管理員 7 個 + 公開 2 個 = 9 個）[規格:L111-L122]
- [x] 9.7.1.2 驗證 FAQ 管理 API（管理員 5 個 + 公開 1 個 = 6 個）[規格:L126-L134]
- [x] 9.7.1.3 驗證資源中心管理 API（管理員 5 個 + 公開 2 個 = 7 個）[規格:L138-L147]
- [x] 9.7.1.4 驗證圖片資源管理 API（管理員 4 個 + 公開 1 個 = 5 個）[規格:L151-L158]
- [x] 9.7.1.5 確認總計：27 個 API（管理員 19 個 + 公開 8 個）

**9.7.2 業務邏輯驗證**
- [x] 9.7.2.1 驗證文章發布規則（草稿/發布/取消發布）[規格:L630-L633]
- [x] 9.7.2.2 驗證 URL Slug 規則（小寫字母、數字、連字號，必須唯一）[規格:L635-L638]
- [x] 9.7.2.3 驗證文件大小限制（圖片 5MB, 資源 10MB）[規格:L640-L643]
- [x] 9.7.2.4 驗證 SEO 優化（標題 50-60 字元，描述 150-160 字元）[規格:L645-L648]
- [x] 9.7.2.5 驗證瀏覽次數自動增加 [規格:L541-L544]
- [x] 9.7.2.6 驗證下載次數自動增加 [規格:L554-L557]

**9.7.3 測試案例執行**
- [x] 9.7.3.1 測試文章發布流程 [規格:L656-L667]
- [x] 9.7.3.2 測試 Slug 唯一性檢查 [規格:L669-L675]
- [x] 9.7.3.3 測試文件大小限制 [規格:L677-L683]
- [x] 9.7.3.4 測試下載次數統計 [規格:L685-L694]

**9.7.4 回到規格逐一驗證**
- [x] 9.7.4.1 打開規格文檔 L11-L37，驗證 ExternalArticles 表完整性
- [x] 9.7.4.2 打開規格文檔 L41-L57，驗證 ExternalFAQ 表完整性
- [x] 9.7.4.3 打開規格文檔 L61-L82，驗證 ResourceCenter 表完整性
- [x] 9.7.4.4 打開規格文檔 L86-L103，驗證 ExternalImages 表完整性
- [x] 9.7.4.5 逐一驗證所有 27 個 API 的實現

---

#### 9.8 部署與測試
- [x] 9.8.1 [內部] 提交所有更改到 Git
- [x] 9.8.2 [內部] 執行自動部署（git push origin main）
- [x] 9.8.3 [內部] 驗證 Cloudflare Pages 部署成功
- [x] 9.8.4 [內部] 更新 MASTER_PLAN.md 進度統計
...
- [x] 12.1.1.2 欄位：`client_id` (TEXT NOT NULL) [規格:L42]
- [x] 12.1.1.3 欄位：`receipt_date`, `due_date` (TEXT) [規格:L43-L44]
- [x] 12.1.1.4 欄位：`total_amount` (REAL NOT NULL, 無稅額) [規格:L45]
- [x] 12.1.1.5 欄位：`status` (TEXT DEFAULT 'unpaid') [規格:L46]
  - CHECK IN ('unpaid','partial','paid','cancelled')
- [x] 12.1.1.6 欄位：`is_auto_generated` (BOOLEAN DEFAULT 1) [規格:L47]
- [x] 12.1.1.7 欄位：`notes`, `created_by` [規格:L48-L49]
- [x] 12.1.1.8 審計與作廢：`created_at`, `updated_at`, `is_deleted`, `deleted_at`, `deleted_by` [規格:L50-L54]
- [x] 12.1.1.9 外鍵：`client_id` REFERENCES Clients(client_id) [規格:L56]
- [x] 12.1.1.10 外鍵：`created_by` REFERENCES Users(user_id) [規格:L57]
- [x] 12.1.1.11 外鍵：`deleted_by` REFERENCES Users(user_id) [規格:L58]
- [x] 12.1.1.12 索引：`idx_receipts_client` ON (client_id) [規格:L61]
- [x] 12.1.1.13 索引：`idx_receipts_date` ON (receipt_date) [規格:L62]
- [x] 12.1.1.14 索引：`idx_receipts_status` ON (status) [規格:L63]
- [x] 12.1.1.15 索引：`idx_receipts_status_due` ON (status, due_date) ⭐應收帳款專用 [規格:L64, L70-L73]
- [x] 12.1.1.16 索引：`idx_receipts_client_status` ON (client_id, status) ⭐客戶收款專用 [規格:L65, L75-L77]

**12.1.2 創建 `ReceiptItems` 表（收據項目）[規格:L80-L97]**
- [x] 12.1.2.1 主鍵 `item_id` (INTEGER PRIMARY KEY AUTOINCREMENT) [規格:L84]
- [x] 12.1.2.2 關聯欄位：`receipt_id` (TEXT NOT NULL) [規格:L85]
- [x] 12.1.2.3 關聯欄位：`service_id` (INTEGER, 可選) [規格:L86]
- [x] 12.1.2.4 欄位：`description` (TEXT NOT NULL) [規格:L87]
- [x] 12.1.2.5 欄位：`quantity` (REAL DEFAULT 1) [規格:L88]
- [x] 12.1.2.6 欄位：`unit_price` (REAL NOT NULL) [規格:L89]
- [x] 12.1.2.7 欄位：`amount` (REAL NOT NULL, = quantity × unit_price) [規格:L90]
- [x] 12.1.2.8 外鍵：`receipt_id` REFERENCES Receipts(receipt_id) ON DELETE CASCADE [規格:L92]
- [x] 12.1.2.9 外鍵：`service_id` REFERENCES Services(service_id) [規格:L93]
- [x] 12.1.2.10 索引：`idx_receipt_items_receipt` ON (receipt_id) [規格:L96]

**12.1.3 創建 `ReceiptSequence` 表（收據流水號管理，含 UNIQUE 約束）[規格:L99-L116]**
- [x] 12.1.3.1 主鍵 `sequence_id` (INTEGER PRIMARY KEY AUTOINCREMENT) [規格:L103]
- [x] 12.1.3.2 欄位：`year_month` (TEXT UNIQUE NOT NULL, 格式：YYYYMM) [規格:L104]
- [x] 12.1.3.3 欄位：`last_sequence` (INTEGER NOT NULL DEFAULT 0) [規格:L105]
- [x] 12.1.3.4 審計：`created_at`, `updated_at` [規格:L106-L107]
- [x] 12.1.3.5 索引：`idx_receipt_sequence_ym` ON (year_month) [規格:L110]

**12.1.4 創建 `Payments` 表（收款記錄）[規格:L118-L138]**
- [x] 12.1.4.1 主鍵 `payment_id` (INTEGER PRIMARY KEY AUTOINCREMENT) [規格:L122]
- [x] 12.1.4.2 關聯欄位：`receipt_id` (TEXT NOT NULL) [規格:L123]
- [x] 12.1.4.3 欄位：`payment_date` (TEXT NOT NULL) [規格:L124]
- [x] 12.1.4.4 欄位：`amount` (REAL NOT NULL) [規格:L125]
- [x] 12.1.4.5 欄位：`payment_method`, `reference_number`, `notes` [規格:L126-L128]
- [x] 12.1.4.6 欄位：`received_by` (INTEGER NOT NULL) [規格:L129]
- [x] 12.1.4.7 審計：`created_at` [規格:L130]
- [x] 12.1.4.8 外鍵：`receipt_id` REFERENCES Receipts(receipt_id) [規格:L132]
- [x] 12.1.4.9 外鍵：`received_by` REFERENCES Users(user_id) [規格:L133]
- [x] 12.1.4.10 索引：`idx_payments_receipt` ON (receipt_id) [規格:L136]
- [x] 12.1.4.11 索引：`idx_payments_date` ON (payment_date) [規格:L137]
...
### 完成統計
- **已完成模組：** 14 / 14（100%）✅ 全部完成！
- **已完成資料表：** 45 / 45（100%）✅ 全部完成！
- **已完成 API：** 147 / 147（100%）✅ 全部完成！
- **已完成 Cron Jobs：** 6 / 6（100%）✅ 全部完成！
...
| 7. 任務管理 | ✅ 已完成 | 2025-10-29 | ✅ 已部署 |
| 8. 知識管理 | ✅ 已完成 | 2025-10-29 | ✅ 已部署 |
| 9. 外部內容管理 | ✅ 已完成 | 2025-10-29 | ✅ 已部署 |
| 10. 薪資管理 | ✅ 已完成 | 2025-10-29 | ✅ 已部署 |
| 11. 管理成本 | ✅ 已完成 | 2025-10-29 | ✅ 已部署 |
| 12. 收據收款 | ✅ 已完成 | 2025-10-29 | ✅ 已部署 |
| 13. 附件系統 | ✅ 已完成 | 2025-10-29 | ✅ 已部署 |
| 14. 報表分析 | ✅ 已完成 | 2025-10-29 | ✅ 已部署 |
