/**
 * blog-modern.js - 霍爾果斯會計師事務所部落格頁面優化腳本
 * 基於現有blog.js進行優化和現代化設計
 * 最後更新日期: 2025-05-07
 */

document.addEventListener('DOMContentLoaded', function() {
  // 輸出調試信息，確認腳本已加載
  console.log('現代化博客頁面腳本已加載');
  ...
  function getUrlParams() {
    ...
    if (endPage < totalPages) {
      ...
      lastPageBtn.setAttribute('aria-label', `第${totalPages}頁`);
  ...
  function useFallbackData() {
    ...
    const fallbackData = {
      ...
      categories: ["tax", "accounting", "business", "investment", "international"],
  ...
  function renderFilteredPosts() {
    ...
    // 顯示總文章數
    if (totalPosts > 0) {
      searchResultsContainer.innerHTML = `<div class="search-results-info"><p>共找到 ${totalPosts} 篇文章</p></div>`;
    } else {
  ...
  function renderPagination(totalPages) {
    ...
    for (let i = startPage; i <= endPage; i++) {
      ...
      pageBtn.setAttribute('aria-label', `第${i}頁`);
