...
**注意**：專案管理資料表 (`projects`, `project_tasks`, `project_checklist`) 目前存在但可能未充分使用。建議未來：
- 選項 1：完全整合到多階段任務系統（推薦）
- 選項 2：保留專案表，作為大型專案的額外管理層
...
├── template-market.js       # 模板市場（新增）
└── template-editor.js       # 拖拽式模板編輯器（新增）

assets/css/
├── template-market.css      # 模板市場樣式（新增）
└── template-editor.css      # 模板編輯器樣式（新增）
```
