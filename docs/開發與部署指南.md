...
1. **第一步：更新設計文檔**
   ...
   - 更新功能描述、資料表結構、API 設計
   - 確保設計完整、清晰、可行
   - **⚠️ 如果發現任何問題或有修改意見，必須將修改建議整合進對應的設計文檔**
...
5. **第五步：自動部署**
   ...
   - **不需要手動執行部署命令**
   - 可在 GitHub Actions 頁面查看部署狀態

### 其他開發原則

#### AI 助理協作原則

- **主動更新設計**: AI 助理在發現任何代碼與設計不符、或有任何優化建議時，**必須**先更新相關設計文檔，再進行編碼。
- **整合修改建議**：AI 助理發現問題或有修改意見時，**必須**將修改建議直接整合進對應的設計文檔，然後再開始修復。
- **自動化優先**: AI 助理**絕不**要求手動部署，所有變更透過 Git Push 觸發自動化流程。
- **發現問題即處理**: AI 助理發現問題後，應直接更新 TODO 列表並著手修復，**不生成額外的問題彙總報告**。
...
#### 問題處理原則
- ✅ 發現問題立即更新 TODO 並修復
- ✅ 發現問題或修改意見時，直接整合進對應的設計文檔
- ❌ **不彙總問題清單**（浪費 token 和增加垃圾文檔）
- ❌ 不在修復前先列出所有問題（發現即修復）
- ✅ 使用 TODO 工具追蹤修復進度

#### 代碼修復流程
1. **完整檢查代碼**：逐文件完整檢查所有代碼，不偷懶
2. **發現問題即記錄**：添加到 TODO 列表
3. **更新設計文檔**：如需修改設計，先更新設計文檔
4. **立即修復問題**：檢查完成後立即開始修復
5. **自動部署生效**：推送到 main 分支後自動部署
