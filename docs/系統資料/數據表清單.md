...
| `ServiceChangeHistory` | 服務變更歷史 | 記錄服務狀態變更（暫停/恢復/取消）⭐ |
...
| `DashboardWidgets` | 儀表板小工具 | 儀表板組件配置 ⭐ |
| `UserDashboardSettings` | 用戶儀表板設定 | 用戶自定義儀表板佈局 ⭐ |
| `TimeLogs_Archive_YYYY` | 工時歸檔表 | 按年份歸檔的歷史工時數據 |
| `LeaveApplications_Archive_YYYY` | 假期歸檔表 | 按年份歸檔的歷史假期數據 |
| `AuditLogs_Archive_YYYY` | 日誌歸檔表 | 按年份歸檔的歷史日誌數據 |

### 薪資管理（5個）⭐
| 表名 | 中文名稱 | 用途 |
|------|---------|------|
| `SalaryItemTypes` | 薪資項目類型 | 定義薪資項目（全勤、津貼、加給等）|
| `EmployeeSalaryItems` | 員工薪資項目 | 員工的具體薪資項目設定 |
| `MonthlyPayroll` | 月度薪資表 | 每月薪資計算結果 |
| `OvertimeRecords` | 加班記錄明細 | 加班時數與補休記錄 |
| `YearEndBonus` | 年終獎金 | 年終獎金記錄 |

### 管理成本（2個）⭐
| 表名 | 中文名稱 | 用途 |
|------|---------|------|
| `OverheadCostTypes` | 管理成本項目類型 | 定義成本項目（租金、水電等）|
| `MonthlyOverheadCosts` | 月度管理成本 | 每月管理成本記錄 |

### 收據收款（4個）⭐
| 表名 | 中文名稱 | 用途 |
|------|---------|------|
| `Receipts` | 收據 | 開立的收據記錄 |
| `ReceiptItems` | 收據項目 | 收據明細項目 |
| `ReceiptSequence` | 收據流水號管理 | 管理收據編號 |
| `Payments` | 收款記錄 | 實際收款記錄 |

### 附件系統（1個）⭐
| 表名 | 中文名稱 | 用途 |
|------|---------|------|
| `Attachments` | 附件 | 各種實體的附件管理（客戶、收據、SOP、任務）|
...
- **薪資項目** → SalaryItemTypes + EmployeeSalaryItems
- **月度薪資** → MonthlyPayroll
- **管理成本** → OverheadCostTypes + MonthlyOverheadCosts
- **收據** → Receipts + ReceiptItems
- **收款記錄** → Payments
- **附件** → Attachments
