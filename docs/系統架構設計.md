...
**資料表**: `clients` (統一後), `client_services`, `client_interactions`  
**設計文檔**: [客戶管理系統設計.md](./客戶管理系統設計.md)

### 5. 專案管理系統
**頁面**: `projects.html` (此頁面已不存在，但功能由JS驅動)
**模組**: `src/projects.js`
**功能**: 獨立的專案管理，用於追蹤大型、非標準的任務，例如公司設立、財稅簽證等。
**資料表**: `projects`
**設計文檔**: [專案管理系統設計.md](./專案管理系統設計.md) (待補)

### 6. 知識庫系統
**頁面**: `knowledge.html` (SOP+內部文檔+FAQ 三個標籤)  
**模組**: `src/sop.js`  
**功能**: SOP 管理、內部文檔、FAQ、版本控制、搜尋  
**資料表**: `sop_categories`, `sops`, `sop_versions`, `faq_categories`, `faqs`
...
**模組**: `src/system-config.js`, `src/workload.js`
...
### ⚠️  abandoned table integration instructions
**專案管理相關資料表**：
- `projects` 資料表為現行專案管理系統使用，並非已整合或待廢棄。
- `tasks` 表中的 `category = 'project'` 是誤用，應直接使用 `projects` 系統。
- `projects.html` 頁面雖已刪除，但其功能可能已整合至其他頁面的動態JS中。
...
CREATE TABLE client_services (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  client_id INTEGER NOT NULL, -- 參照 clients(id)
...
CREATE TABLE timesheets (
  ...
  client_id INTEGER, -- 參照 clients(id)
  date DATE NOT NULL,
  ...
  UNIQUE(employee_id, date, client_id)
);
```
...
#### 統一遷移檔案
單一遷移檔案 `001_initial_schema.sql` 已包含所有資料表的最終結構。
