...
('comp_leave_expiry_rule', 'current_month', '補休有效期規則（current_month/next_month/3_months/6_months）', 1),  -- ⚠️ 危險設定
...
- 認證：登入/登出/修改密碼流程可用，未登入訪問受保護路由被攔截
- 權限：`is_admin`=0 無法訪問 `/api/v1/admin/*` 端點
- 審計：關鍵操作寫入 `AuditLogs`，欄位正確
- 軟刪除：資料標記 `is_deleted=1` 後預設查詢不可見，可復原
- 歸檔：年度歸檔表存在並可查詢歷史資料
