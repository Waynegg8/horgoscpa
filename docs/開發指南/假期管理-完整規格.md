...
CREATE TABLE Notifications (
  ...
  user_id INTEGER NOT NULL,              -- 通知對象
  type TEXT NOT NULL,                    -- 'missing_timesheet', 'task_overdue', 'comp_leave_expiring', etc.
  message TEXT NOT NULL,                 -- 通知訊息
  related_date TEXT,                     -- 關聯日期（如：未填工時的日期）
  related_id INTEGER,                    -- 關聯ID（如：task_id, leave_id）
  action_url TEXT,                       -- 操作連結
  is_read BOOLEAN DEFAULT 0,             -- 是否已讀
  auto_dismiss BOOLEAN DEFAULT 0,        -- 是否自動消除（0=需手動處理，1=可自動過期）
  created_at TEXT DEFAULT (datetime('now')),
  dismissed_at TEXT,                     -- 消除時間
