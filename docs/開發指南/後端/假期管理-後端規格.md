...
### 2.6 CronJobExecutions / Notifications
...
### 3.9 年初處理與歷史（管理員）
- `POST /admin/cron/execute`（指定 `annual_leave_update`）
- `GET /admin/cron/history?job_name=annual_leave_update`
...
- 補休
  - 依『工時/補休』規則：每月最後一天將剩餘補休轉換為加班費並清零（細節見補休模組）
- 國定假日
